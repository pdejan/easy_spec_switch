...
local function setupColors()
    ...
    local accent = color and {color.r, color.g, color.b, 1} or {0.7, 0.7, 0.7, 1}
    local luminance = 0.2126 * linear(accent[1]) + 0.7152 * linear(accent[2]) + 0.0722 * linear(accent[3])
    local darkText = luminance > 0.179
    C.accent = accent
    C.primaryText = darkText and {0, 0, 0, 1} or {1, 1, 1, 1}
    C.hoverAccent = {0, 0, 0, 1}
    for i = 1, 3 do
        C.hoverAccent[i] = darkText and (accent[i] + (1 - accent[i]) * 0.12) or accent[i] * 0.88
    end
end
...
local function highlight(frame, active)
    ...
    if frame.essActive == active then return end
    frame.essActive = active
...
local function button(parent, text, width, height, primary, dropdown)
    ...
    b.caption:SetTextColor(unpack(primary and C.primaryText or C.text))
    if primary then
    ...
    b:SetScript("OnEnter", function(self)
        ...
        if primary then self:SetBackdropColor(unpack(C.hoverAccent)) end
    end)
...
function ESS:ShowConfig()
    if self.window then self.window:Show(); return end
    setupColors()
    ...
    for i, spec in ipairs(specs) do
        ...
        local edit = editBox(row, 156, 28, 22)
    ...
    for slot = 1, self.slotCount do
        ...
        local number = editBox(window, 120, 34, 8)
    ...
    function window:Refresh()
        ...
        for slot, row in ipairs(self.buildRows) do
            ...
            local number = draft and draft[slot] or ESS:BuildNumber(specID, slot)
            -- Avoid resetting the cursor while the player is typing.
            if row.number:GetText() ~= number then row.number:SetText(number) end
    ...
    local events = {"PLAYER_SPECIALIZATION_CHANGED", "TRAIT_CONFIG_LIST_UPDATED",
        "SELECTED_LOADOUT_CHANGED", "TRAIT_CONFIG_UPDATED", "ACTIVE_COMBAT_CONFIG_CHANGED"}
    local function onShow(self)
        for _, event in ipairs(events) do self:RegisterEvent(event) end
        self:Refresh()
    end
    window:SetScript("OnShow", onShow)
    window:SetScript("OnEvent", window.Refresh)
